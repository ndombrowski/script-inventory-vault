...
out1=05_quality_filtering/${sample_id}_R1_trim.fastq.gz
out2=05_quality_filtering/${sample_id}_R2_trim.fastq.gz
...
if [[ -f "$out1" && -f "$out2" ]]; then
    ...
    fastp \
        ...
        --thread 20 \
        --html 05_quality_filtering/${sample_id}.fastp-trim.report.html \
        --out1 $out1 \
        --out2 $out2
